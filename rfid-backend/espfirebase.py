...
def home():
    ...
    # If you have an index.html in templates, render it; otherwise return a JSON status
    if os.path.exists(os.path.join('templates', 'index.html')):
        return render_template('index.html')
    return jsonify({"status": "ok", "message": "RFID server"})


@app.route('/rfid/card', methods=['POST'])
def receive_rfid():
    """Endpoint to receive RFID card data from ESP8266 or other devices"""
